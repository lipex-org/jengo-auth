...
<body>
    <div class="card">
        ...
        <p style="font-size: 0.875rem; color: #64748b;">Enter your email to receive a password reset link.</p>
        <?php if (session()->getFlashdata('message')): ?>
            <div class="success"><?= esc(session()->getFlashdata('message')) ?></div>
        <?php endif; ?>
        <form action="<?= url_to('forgot-password.send') ?>" method="post">
            <?= csrf_field() ?>
        ...
        <div class="links">
            <a href="<?= url_to('login') ?>">Back to Log In</a>
        </div>
    </div>
</body>
</html>
